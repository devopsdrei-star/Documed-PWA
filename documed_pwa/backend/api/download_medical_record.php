...
if ($isPdf) {
    ...
    header('Content-Disposition: attachment; filename="medical-record-' . ($sid ? preg_replace('/[^A-Za-z0-9_-]/', '-', $sid) : $checkupId) . '.pdf"');
...
if ($isPdf) {
  if (!class_exists('Dompdf\Dompdf')) {
    $vendorRoot = dirname(__DIR__, 3) . '/vendor/autoload.php'; // project root
    $vendorLocal = dirname(__DIR__, 2) . '/vendor/autoload.php'; // documed_pwa/vendor
